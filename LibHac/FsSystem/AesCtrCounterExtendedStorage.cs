...
public class AesCtrCounterExtendedStorage : IStorage
{
    ...
    {
        using UniqueRef<IDecryptor> decryptor = new(new ExternalDecryptor(decryptFunction, keyIndex, keyGeneration));
    ...
    {
        using UniqueRef<IDecryptor> decryptor = new(new SoftwareDecryptor());
        ...
        ref readonly ValueSubStorage dataStorage, ref readonly ValueSubStorage tableStorage)
    {
        ...
        using UniqueRef<IDecryptor> decryptor = new();
        ...
        using ValueSubStorage entryStorage = new(in tableStorage, entryStorageOffset, entryStorageSize);
        using ValueSubStorage nodeStorage = new(in tableStorage, nodeStorageOffset, nodeStorageSize);
    ...
    public void FinalizeObject()
    {
        if (IsInitialized())
        {
            ...
            using ValueSubStorage emptyStorage = new();
    ...
    public override Result Read(long offset, Span<byte> destination)
    {
        ...
        using ScopedThreadPriorityChanger changePriority = new(1, ScopedThreadPriorityChanger.Mode.Relative);
    ...
    private class ExternalDecryptor : IDecryptor
    {
        ...
        public Result Decrypt(Span<byte> destination, ReadOnlySpan<byte> encryptedKey, ReadOnlySpan<byte> iv)
        {
            ...
            using PooledBuffer pooledBuffer = new();
