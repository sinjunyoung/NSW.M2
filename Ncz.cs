...
public class Ncz : Nca
{
    ...
    {
        ArgumentNullException.ThrowIfNull(nczStream);

        // Read NCZ header
        var nczHeader = NczHeader.Read(nczStream);

        nczToNcaStream = readMode switch
        {
            NczReadMode.Original => CreateOriginalNczToNcaStream(nczHeader, nczStream),
            NczReadMode.Fast => CreateFastNczToNcaStream(nczHeader, nczStream, keySet),
