...
public class NczSectionsEncryptionHelper(IReadOnlyList<NczSection> nczSections)
{
    private readonly IReadOnlyList<NczSection> _nczSections = nczSections ?? throw new ArgumentNullException(nameof(nczSections));
...
/// </summary>
public class SectionEncryptionHelper(NczSection section) : IDisposable
{
    private readonly Aes128CtrTransform? _crypto = section.CryptoType switch
    {
        NcaEncryptionType.Auto or NcaEncryptionType.None => null,
        NcaEncryptionType.AesCtr or NcaEncryptionType.AesCtrEx => new Aes128CtrTransform(section.CryptoKey, section.CryptoCounter),
        _ => throw new NotSupportedException($"Encryption type \"{section.CryptoType}\" not supported."),
    };
    ...
    public int Recover(long ncaOffset, Span<byte> buffer)
    {
        if (!section.ContainsOffset(ncaOffset))
        ...
        var nbAvailableSectionBytes = section.Offset + section.Size - ncaOffset;
